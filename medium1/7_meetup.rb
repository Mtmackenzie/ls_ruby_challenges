# Meetup
=begin
Problem: find the date by iterating from the given date up to the end of the given criteria until the criteria is met
Rules: 
DS: Meetup class objects

Algo: 
day method: takes week symbol and day symbol
Start with given date: month, num, year.
num: depends on given :week. :week will have range of nums. iterate through given nums until meets criteria.
day criteria: convert day to a string, add ?, make symbol, save method symb to proc in lv day, pass each to block with method that checks if the day is that day of the week
=end
...
class Meetup
  ...
  def day(day, week)
    @week = find_range(week)
    @day = day
    check_if_valid = method(:weekday).to_proc
    @week.each(&check_if_valid)
    @answer
  end
  ...
  def weekday(num)
    ...
    new_date = date(num)
    schedule = (@day.to_s + '?').to_sym
    check = new_date.method(schedule)
    @answer = new_date if check.call
  end
