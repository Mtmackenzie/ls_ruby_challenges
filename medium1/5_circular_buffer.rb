=begin 
Problem: don't let an array go over size. create methods to add and remove elements from array: adding to the end and removing from front.
DS: array
Algo: 
# constructor: number passed in is the size of the buffer
# read method: removes oldest (first element in array)
- raise error if empty when calling read: CircularBuffer::BufferEmptyException
# write method: adds element to end of buffer
- if nil, should not occupy buffer
- CircularBuffer::BufferFullException if buffer is full when called
# write! removes first element and adds new element if full, otherwise behaves like write
- if nil, should not occupy buffer
# clear erases all elements in the buffer
=end
