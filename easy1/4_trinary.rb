...
# Input: string
# Output: 
# Rules: validate - can only contain 1 or 0 or 2.
# Algo: 
# validate string input
# convert to array of digits (reversed) and iterate over with index, multiply number by 3 to the index power, sum all.
...
class Trinary
  ...
  def validate(string)
    string.chars.map { |char| '012'.include?(char) ? char : (return 0)}.join.to_i
  end

  def to_decimal
    sum = 0
    @number.digits.each_with_index { |num, idx| sum += (num * (3 ** idx)) }
    sum
  end
end
