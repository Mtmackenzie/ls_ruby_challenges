...
# Output: array of all possible n length sets in integer form as individual elements in arrays (consecutive)
# Rules: raise argument error if n is greater than string's size
# Problem: convert the string to an array of characters, convert all chars to integers, and collect n amount of consecutive chars and return in an array of nested arrays
# create Series class with slices instance method that takes an integer as an argument and works with the string value passed in to the constructor
# DS: string, arr
# 
# Algo: 
# 
...
class Series
  ...
  def slices(n)
    ...
    @string.chars.map(&:to_i).each_cons(n) do |num|
      result << num
    end
    result
  end
